...
bool rgb_color_t::try_parse_special(const wcstring &special)
{
    ...
    if (! std::wcscasecmp(name, L"normal"))
    {
        this->type = type_normal;
    }
    else if (! std::wcscasecmp(name, L"reset"))
    {
        this->type = type_reset;
    }
    else if (! std::wcscasecmp(name, L"ignore"))
    {
...
bool rgb_color_t::try_parse_named(const wcstring &str)
{
    ...
    {
        if (0 == std::wcscasecmp(str.c_str(), named_colors[idx].name))
        {
