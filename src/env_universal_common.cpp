...
/*
 * For the fucking solaris.
 * Modified by Loutzongyi Lee.
 */

#include "flock.h"

/*
 * Copy from linux header file <bits/mman-linux.h>
 *
 */
#ifndef MAP_FILE
# define MAP_FILE 0
...
static bool match(const wchar_t *msg, const wchar_t *cmd)
{
    ...
    if (std::wcsncasecmp(msg, cmd, len) != 0)
        return false;
...
bool env_universal_t::open_and_acquire_lock(const wcstring &path, int *out_fd)
{
    ...
    */
	...
	/*
	 * Replace to _flock().
	 * In solaris there are an struct flock existed qwq
	 * Really damn.
	 *
	 */
            while (_flock(fd, LOCK_EX) < 0)
            {
                /* error */
                if (errno != EINTR)
                {
