...
{
    size_t prefix_size = proposed_prefix.size();
    return prefix_size <= value.size() && std::wcsncasecmp(proposed_prefix.c_str(), value.c_str(), prefix_size) == 0;
}
...
string_fuzzy_match_t string_fuzzy_match_string(const wcstring &string, const wcstring &match_against, fuzzy_match_type_t limit_type)
{
    ...
    else if (limit_type >= fuzzy_match_case_insensitive && std::wcscasecmp(string.c_str(), match_against.c_str()) == 0)
    {
